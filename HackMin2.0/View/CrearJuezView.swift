//
//  CrearJuezView.swift
//  HackMin2.0
//
//  Created by Naolop on 08/05/26.
//

import SwiftUI

struct CrearJuezView: View {
    @StateObject private var vm = CrearJuezViewModel()
    @Binding var isPresented: Bool
    var idConcurso: String = ""
    var onGuardar: (JuezModel) -> Void

    var body: some View {
        GeometryReader { geo in
            ZStack {
                Image("Diseño7")
                    .resizable()
                    .scaledToFill()
                    .ignoresSafeArea()

                // Card glassmorphism
                RoundedRectangle(cornerRadius: 28, style: .continuous)
                    .fill(.ultraThinMaterial)
                    .overlay(
                        RoundedRectangle(cornerRadius: 28)
                            .stroke(Color.white.opacity(0.5), lineWidth: 1)
                    )
                    .frame(
                        width: geo.size.width * 0.65,
                        height: geo.size.height * 0.72
                    )

                HStack(spacing: 0) {
                    // Lado izquierdo — ícono
                    VStack(spacing: geo.size.height * 0.025) {
                        ZStack {
                            Circle()
                                .fill(Color.orange.opacity(0.2))
                                .frame(
                                    width: geo.size.width * 0.14,
                                    height: geo.size.width * 0.14
                                )
                            Image(systemName: "person.badge.shield.checkmark.fill")
                                .font(.system(size: geo.size.width * 0.045, weight: .semibold))
                                .foregroundColor(.orange)
                        }
                        Text("Nuevo juez")
                            .font(.system(
                                size: geo.size.width * 0.014,
                                weight: .semibold,
                                design: .rounded
                            ))
                            .foregroundColor(.gray)
                    }
                    .frame(width: geo.size.width * 0.65 * 0.35)

                    // Divider
                    Rectangle()
                        .fill(Color.white.opacity(0.4))
                        .frame(width: 1, height: geo.size.height * 0.45)

                    // Lado derecho — formulario
                    VStack(spacing: geo.size.height * 0.035) {

                        CustomTextField(
                            placeholder: "Alias del juez",
                            text: $vm.alias,
                            type: .normal,
                            backgroundColor: .white.opacity(0.75),
                            foregroundColor: .black,
                            tittleColor: .black,
                            cornerRadius: 30,
                            height: geo.size.height * 0.09,
                            width: geo.size.width * 0.65 * 0.52
                        )

                        CustomTextField(
                            placeholder: "Código del juez",
                            text: $vm.codigoJuez,
                            type: .normal,
                            backgroundColor: .white.opacity(0.75),
                            foregroundColor: .black,
                            tittleColor: .black,
                            cornerRadius: 30,
                            height: geo.size.height * 0.09,
                            width: geo.size.width * 0.65 * 0.52
                        )

                        if vm.mostrarError {
                            Text(vm.mensajeError)
                                .font(.system(size: geo.size.width * 0.013))
                                .foregroundColor(.red)
                        }

                        CustomButton(
                            action: {
                                
                                vm.guardarJuez(idConcurso: idConcurso)
                                
                                
//                                if let juez = vm.guardarJuez(idConcurso: idConcurso) {
//                                    onGuardar(juez)
//                                    isPresented = false
//                                }
                            },
                            style: .standard(
                                fontColor: .white,
                                backgroundColor: .orange,
                                buttonName: "Agregar juez",
                                fontName: "Helvetica Neue",
                                fontSize: geo.size.width * 0.018,
                                width: geo.size.width * 0.65 * 0.52,
                                height: geo.size.height * 0.09
                            )
                        )
                    }
                    .frame(width: geo.size.width * 0.65 * 0.58)
                    .padding(.vertical, geo.size.height * 0.04)
                }
                .frame(
                    width: geo.size.width * 0.65,
                    height: geo.size.height * 0.72
                )
            }
            .frame(width: geo.size.width, height: geo.size.height)
        }
    }
}

#Preview {
    CrearJuezView(isPresented: .constant(true)) { _ in }
        .previewInterfaceOrientation(.landscapeLeft)
}
